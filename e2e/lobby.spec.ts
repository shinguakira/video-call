...
test.describe("Lobby page", () => {
  ...
  test("shows heading to check audio and video", async ({ page }) => {
    await expect(page.getByRole("heading", { name: /Check your audio and video/i })).toBeVisible();
  });
  ...
  test("Join Meeting button is disabled / does nothing with empty name", async ({ page }) => {
  ...
  test("Join Meeting navigates to room when name is provided", async ({ page }) => {
    await page.getByLabel("Display Name").fill("Alice");
    await page.getByRole("button", { name: /Join Meeting/i }).click();
    await expect(page).toHaveURL(`/room/${ROOM_ID}?name=${encodeURIComponent("Alice")}`);
  });
  ...
  test("shows invalid room message when roomId is missing", async ({ page }) => {
    await page.goto("/lobby");
    await expect(page.getByText(/Invalid room ID/i)).toBeVisible();
  });

  test('"Go Home" button on invalid-room page navigates to /', async ({ page }) => {
    await page.goto("/lobby");
