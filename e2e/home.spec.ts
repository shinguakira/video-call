...
test.describe("Home page", () => {
  ...
  test("renders hero headline", async ({ page }) => {
    await expect(page.getByRole("heading", { name: /Premium Video Meetings/i })).toBeVisible();
  });
  ...
  test('"New Meeting" navigates to /lobby with a generated roomId', async ({ page }) => {
    await page.getByRole("button", { name: /New Meeting/i }).click();
    await expect(page).toHaveURL(/\/lobby\?roomId=\w+/);
  });

  test("Join form navigates to /lobby with the entered roomId", async ({ page }) => {
  ...
  test("Join form does not navigate when room ID is empty", async ({ page }) => {
    await page.getByRole("button", { name: /^Join$/i }).click();
