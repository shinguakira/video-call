  ...
  useEffect(() => {
    ...
    const handleExistingUsers = (existingUsers: Array<{ userId: string; userName: string }>) => {
      ...
      console.log(
        ...
        "peer connections as NON-INITIATOR",
      );
      ...
      console.log(
        "[CLIENT] Finished processing existing users, peersRef has:",
    ...
    const handleSignal = ({ fromUserId, signal }: { fromUserId: string; signal: any }) => {
