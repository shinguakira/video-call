      ...
      <main className="flex-1 relative flex">
        ...
        <div className="absolute top-4 left-4 bg-black/80 text-white p-4 rounded-lg text-xs font-mono z-50 pointer-events-none max-w-sm">
          ...
          <div className="space-y-1">
            <p>
              <span className="text-muted-foreground">Room ID:</span> {roomId}
            </p>
            <p>
              <span className="text-muted-foreground">Local User:</span> {userName} (
              {userId.slice(0, 4)}...)
            </p>
            <p>
              <span className="text-muted-foreground">Total Participants:</span> {peers.size + 1}
            </p>
              ...
              {peers.size === 0 ? (
                ...
                <ul className="list-disc pl-4 space-y-0.5">
                  {Array.from(peers.values()).map((peer) => (
                    <li key={peer.peerId}>
                      {peer.userName}{" "}
                      <span className="text-muted-foreground">({peer.peerId.slice(0, 4)}...)</span>
                    </li>
                  ))}
                </ul>
              )}
